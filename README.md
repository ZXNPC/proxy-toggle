...
- 🎨 图形化设置：代理地址、快捷键、提示开关、透明度/字号滑块（提示框尺寸随字号自动计算），**设置界面内实时预览提示框样式**，支持一键还原默认
...
- **快捷键**：点击"监听..."后按下新的组合键即可捕获；输入框以可读形式显示（如 `Ctrl+Alt+P`），也支持直接输入 AHK 语法（如 `^!p`）
- **显示提示**：是否显示右下角状态指示器
- **提示透明度 / 字号**：滑块调节（透明度 0–255、字号 8–48）；提示框宽度/高度由字号自动计算
- **提示框预览**：设置窗口内实时预览提示框样式（黑底绿字、字号、尺寸），随滑块即时变化
